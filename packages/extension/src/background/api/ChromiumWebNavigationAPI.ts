 ...
 * @license Apache-2.0
 * @file ChromiumWebNavigationAPI.ts
 * @description 
 * Provide Chromium webNavigation APIs
...
/**
 * The webNavigation api wrapper on chromium based browsers
 */
export class ChromiumWebNavigationAPI extends EventEmitter<WebNavigationEvents> {
  constructor() {
    super();
    this._init();
  }
      ...
      chrome.webNavigation.getAllFrames({ tabId: tabId }, (frames) => {
        ...
        if (error) {
          ...
          const result: FrameResult[] = frames.map((frame) => {
            return { tabId: tabId, ...frame } as FrameResult;
          });
          resolve(result);
        }
      });
